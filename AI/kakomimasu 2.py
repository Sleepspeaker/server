...
class SampleAI:
    ...
    def _get_random_actions(self, match: Match): # この関数がランダムだけど行動決めてる
        ...
        for agent in me.agents:
            candidates = [
                action for action in self._enumerate_actions(match, agent) if not to_conflict(action)
            ]
            weights = [
                max(self._eval_weight(match, action), 0) for action in candidates
            ]

            # テストで、1つ目のエージェントを左上に置くようにしてみている
            if agent.on_board(): # エージェントがボード上に居たらデフォ
                action = max(candidates, key=lambda x: self._eval_weight(match, x))
            else: # いなかったら
            ...
            actions.append( # idは1スタートってわけではない、どこにその行動をするかを座標に入れる
                {
                    ...
                    "y": action.pos.y,
                    "type": action.type,
                    "point": self._eval_weight(match, action) #FIXME: 小数の点数が表示される
                }
            )
            i += 1
...
# 行動の評価するみたいよ
    def _eval_weight(self, match: Match, action: ActionRequest) -> int:
        ...
        if action.type == "put":
            ...
            # return neighbor
            return self._eval_square(match, action.pos)
...
# マスの評価
    def _eval_square(self, match: Match, pos: Pos) -> int:
        ...
        if wall != 0:
            return 0
        # 指定した座標が陣地ではなかったら点数を返す
        if area == 0:
            return point
        # 指定した座標が相手の陣地だったら点数を2倍
        if area != self._team_id:
            return point * 2
        # 指定した座標が自分の陣地だったら点数を半分
        else:
            return point * -2
